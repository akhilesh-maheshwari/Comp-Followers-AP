...
try {
  ...
  const boomerangStatUrl   = 'https://s1.boomerangserver.co.in/webhook/private-follower-scraper-stats';

  console.log('Tag Name         :', serviceTagName);
  console.log('Service          :', serviceName);
  console.log('Company URL      :', linkedinCompanyUrl);
  console.log('Followers Count  :', followersCount);

  if (!linkedinCompanyUrl.trim())                           throw new Error('linkedinCompanyUrl is required!');
  if (!linkedinCompanyUrl.includes('linkedin.com/company')) throw new Error('linkedinCompanyUrl must be a valid LinkedIn company page URL!');
  if (isNaN(followersCount) || followersCount <= 0)         throw new Error('followersCount must be a positive number!');
  ...
  while (batchJobs && batchJobs.length > 0) {
          ...
          await fetch(
            'https://frontend.boomerangserver.co.in/webhook/waterfall-output-socialurl',
            {
              ...
              body   : JSON.stringify({
                ...
                requestStatus     : 'Error',
                driveInputLink,
                boomerangOutputUrl: `https://s1.boomerangserver.co.in/webhook/private-follower-scraper-output?request_id=${request_id}`,
                batch_number,
                request_unique_id,
                batchFolderId,
                service_option_1  : serviceOption1,
    ...
    for (const result of batchStatusResults) {
      ...
      console.log(`  💳 Batch ${batch_number} — Charging for ${batchSize} followers ($${batchCost}). Total charged: $${totalCharged.toFixed(3)}`);
      await Actor.charge({ eventName: serviceOption1, count: batchSize });

      const boomerangOutputUrl = `https://s1.boomerangserver.co.in/webhook/private-follower-scraper-output?request_id=${request_id}`;
      ...
      batchResults.push({
        ...
        followers_found: result.followers_found || 0,
  ...
  console.log('LinkedIn Company URL:', linkedinCompanyUrl);
  console.log('Followers Requested :', followersCount);
